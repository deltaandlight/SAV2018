...
typedef struct {
    ...
    PetscInt    curN;
    PetscScalar eps, gamma, c0;
	PetscScalar R, Rold, Rold2;
...
PetscScalar    BDFi(void *ptr, PetscReal*);
...
{                        
	...
	-128, -128, PETSC_DECIDE, PETSC_DECIDE, 1, 3, 0, 0, &user.da);CHKERRQ(ierr);
...
{
    PetscInt       i, j, Mx, My, xs, ys, xm, ym, use_random=1;
    PetscErrorCode ierr;
    PetscReal      L, hx, hy;
    PetscScalar    **x;
    PetscFunctionBeginUser;
	...
	if (use_random)
	{
	    ...
	    PetscRandomSetInterval(rctx, 0.00, 0.14);
...
PetscErrorCode SetKSP(void *ptr){
	...
	PetscInt       	i,j,xe,ye;
	PetscScalar		dt,h;
	PetscReal		pardt,par00,par01,par02,par11,par12,par03,h2,h4,h6;
	DMDALocalInfo	info;
    ...
    par00 = 1.0-((-4.0)/h2 + 2.0*(20.0)/h4 +(-112.0)/h6)*pardt*dt;
    par01 = 	-((1.0)/h2 + 2.0*(-8.0)/h4 +  (57.0)/h6)*pardt*dt;
    par11 = 	-((0.0)/h2 + 2.0* (2.0)/h4 + (-24.0)/h6)*pardt*dt;
    par02 = 	-((0.0)/h2 + 2.0* (1.0)/h4 + (-12.0)/h6)*pardt*dt;
    par12 = 	-((0.0)/h2 + 2.0* (0.0)/h4 +   (3.0)/h6)*pardt*dt;
    par03 = 	-((0.0)/h2 + 2.0* (0.0)/h4 +   (1.0)/h6)*pardt*dt;
   	/*------------------------------------------*/
	for (j = info.ys; j < ye; ++j) {
		for(i = info.xs; i<xe; ++i){
			...
			col[ncols].j = j+1; 	col[ncols].i = i-1;   	v[ncols++] = par11;
			/*(1,2)点*/
			col[ncols].j = j-1;   	col[ncols].i = i-2; 	v[ncols++] = par12;
			col[ncols].j = j+1;   	col[ncols].i = i+2; 	v[ncols++] = par12;
			col[ncols].j = j-1; 	col[ncols].i = i+2;   	v[ncols++] = par12;
			col[ncols].j = j+1; 	col[ncols].i = i-2;   	v[ncols++] = par12;
			/*(2,1)点*/
			col[ncols].j = j-2;   	col[ncols].i = i-1; 	v[ncols++] = par12;
			col[ncols].j = j+2;   	col[ncols].i = i+1; 	v[ncols++] = par12;
			col[ncols].j = j-2; 	col[ncols].i = i+1;   	v[ncols++] = par12;
			col[ncols].j = j+2; 	col[ncols].i = i-1;   	v[ncols++] = par12;
			/*(0,3)点*/
			col[ncols].j = j-0;   	col[ncols].i = i-3; 	v[ncols++] = par03;
			col[ncols].j = j+0;   	col[ncols].i = i+3; 	v[ncols++] = par03;
			col[ncols].j = j-3; 	col[ncols].i = i+0;   	v[ncols++] = par03;
			col[ncols].j = j+3; 	col[ncols].i = i-0;   	v[ncols++] = par03;
...
PetscErrorCode CalB(void *ptr){
    ...
    PetscScalar		eps, h, h2, h4;
    PetscReal		rt_np1_2, rt_n;
	PetscReal		Lapphi, LapLapphi;
    ...
    for (j = ys; j < ye; ++j) {
        for (i = xs; i < xe; ++i) {
            rt_np1_2 = (phi_ex[j][i]*phi_ex[j][i] - eps);
            rt_n = (phi[j][i]*phi[j][i]-eps); 
			Lapphi = ( phi[j+1][i] 
			         + phi[j-1][i] 
					 + phi[j][i+1] 
					 + phi[j][i-1] 
			...
			aB[j][i] = rt_np1_2 * phi_ex[j][i];	                /*the B is U*/
			
            GE1_np1_2 += h2*rt_np1_2*rt_np1_2/(4.0);	                /*local E1n+1/2*/
            GE1       += h2*rt_n*rt_n/(4.0);	                        /*local E1*/		
			GE        += h2*rt_n*rt_n/(4.0)
			+ h2*phi[j][i]*(LapLapphi + 2.0*Lapphi + phi[j][i])/2.0;    /*local E*/
            GBxphi    += h2*phi_sum[j][i]*aB[j][i];                     /*local B*phi*/
        }
    }
...
PetscErrorCode CalC(void *ptr){
    ...
    switch(user->curN){
        ...
        case 1:
            pardt = 2.0/3.0;
            R_sum = 4.0/3.0*user->R + 1.0/3.0*user->Rold;
            break;
        default:
        	R_old[0] = user->R;
			R_old[1] = user->Rold;
...
PetscErrorCode phi_ex(void *ptr){
    ...
    ierr = DMGlobalToLocalBegin(user->da, user->xold, INSERT_VALUES, localxold2);CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(user->da, user->xold, INSERT_VALUES, localxold2);CHKERRQ(ierr);
	...
	switch(user->curN){
  		...
  		case 1:
  			for (j = ys; j < ye; ++j) {
        		for (i = xs; i < xe; ++i) {
            		ax_ex[j][i]  = 2.0*phi[j][i]-alocalxold[j][i];
            		phi_old[0]   = phi[j][i];
					phi_old[1]   = alocalxold[j][i];
					phi_old[2]   = alocalxold[j][i];
            		ax_sum[j][i] = BDFi(user,phi_old);
        		}
    		}
  			break;
  		default:
  			for (j = ys; j < ye; ++j) {
        		for (i = xs; i < xe; ++i) {
            		ax_ex[j][i]  = 3.0*phi[j][i]-3.0*alocalxold[j][i]+alocalxold2[j][i];
            		phi_old[0]   = phi[j][i];
					phi_old[1]   = alocalxold[j][i];
					phi_old[2]   = alocalxold2[j][i];
            		ax_sum[j][i] = BDFi(user,phi_old);
        		}
    		}
	}
...
PetscScalar BDFi(void *ptr,PetscReal* phi_old){
    AppCtx      *user = (AppCtx*)ptr;
    PetscReal	phi_sum;
