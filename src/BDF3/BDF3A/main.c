...
typedef struct {
	...
	KSP			ksp;
	DM			da;
	Vec			x,xold,B,C,X,Y,b;
	Mat			A;
	
    PassiveReal Length;          /* test problem parameter */
    PetscInt   curN;
    PetscScalar beta,eps,R,Rold,gamma,c0,stept[20000];
    PetscScalar	SE1,SE,Tenergy[20000];/*about energy*/
...
extern PetscScalar boundary(PetscScalar,PetscScalar);
...
PetscErrorCode SetKSP(void *ptr){
  	...
  	tsCtx          	*ts = user->ts;
  	PetscErrorCode 	ierr;
	PetscInt       	i,j,xe,ye,Is,Js;
	PetscScalar		dt,eps,beta,h;
	PetscReal		par00,par01,par02,par11;
	DMDALocalInfo	info;
    ...
    par00=1+dt*(4.0*beta/(eps*eps*h*h)+20.0/(h*h*h*h))*2/3.0;
    par01=dt*(-beta/(eps*eps*h*h)-8.0/(h*h*h*h))*2.0/3.0;
    par11=2.0*dt/(h*h*h*h)*2.0/3.0;
    par02=1.0*dt/(h*h*h*h)*2.0/3.0;
   	/*------------------------------------------*/
...
PetscErrorCode CalB(void *ptr){
	...
	PetscReal		rtemp,Lapphi,GE1,GE,GBxphi;/*temp*/
    ...
    /*----point to x,B,----------------------------------------------*/
    PetscScalar **phi,**aB,**alocalxold;/*phi=local_x*/
    ...
    for (j=ys; j<ye; j++) {
        for (i=xs; i<xe; i++) {
            rtemp=(phi[j][i]*phi[j][i]-1.0-beta);            		
            aB[j][i] = rtemp*phi[j][i]/(eps*eps);	/*the B is U*/
            
            Lapphi=(phi[j+1][i]+phi[j-1][i]+phi[j][i+1]+phi[j][i-1]-4.0*phi[j][i])/(h*h);		
            GE1=GE1+h*h*rtemp*rtemp/(eps*eps*4.0);	/*local E1*/
            GE=GE1+h*h*phi[j][i]*(phi[j][i]*beta/(eps*eps)-Lapphi)/2.0;
            		
            GBxphi=GBxphi+(4*phi[j][i]-alocalxold[j][i])*aB[j][i]*h*h;
        }
    }
 	...
 	ierr = PetscPrintf(PETSC_COMM_WORLD,"R = %g\n",user->R);CHKERRQ(ierr);
 	user->SBxphi=user->SBxphi/user->R;
 	...
 	ierr = VecScale(user->B,1./user->R);CHKERRQ(ierr);
...
PetscErrorCode CalC(void *ptr){
	...
	Vec				localx,localxold,localB,LapB,localX;
	//PetcsInt		Ii,Jj;
    ...
    /*----point to x,B,C----------------------------------------------*/
    PetscScalar **phi,**alocalB,**aC,**aLapB,**alocalX,**alocalxold;/*phi=local_x*/ 
    ...
    for (j=ys; j<ye; j++) {
        for (i=xs; i<xe; i++) {           
			...
			-4.0*alocalB[j][i])/(h*h);
            aC[j][i]=(4.0*phi[j][i]-alocalxold[j][i])/3.0+2*ts->dt/9.0*aLapB[j][i]*(4*user->R -user->Rold - user->SBxphi/2.0);	
        }
    }
    ierr = DMDAVecRestoreArray(user->da,LapB,&aLapB);CHKERRQ(ierr);
    
    ierr = KSPSolve(user->ksp,LapB,user->X);CHKERRQ(ierr);	/*得到的是一个中间参数,用来算gamma的，暂时用一下X*/
	ierr = DMGetLocalVector(user->da,&localX);CHKERRQ(ierr);
	ierr = DMGlobalToLocalBegin(user->da,user->X,INSERT_VALUES,localX);CHKERRQ(ierr);
    ierr = DMGlobalToLocalEnd(user->da,user->X,INSERT_VALUES,localX);CHKERRQ(ierr);	
	...
	for (j=ys; j<ye; j++) {
        for (i=xs; i<xe; i++) {
            Lgamma = -alocalB[j][i]*alocalX[j][i];
...
PetscErrorCode Calnewx(void *ptr){
    ...
    for (j=ys; j<ye; j++) {
        for (i=xs; i<xe; i++) {
            LBxphiP = alocalB[j][i]*alocalY[j][i]/(1+ts->dt*user->gamma/3.0);
    ...
    ierr = MPI_Allreduce(&GBxphiP, &user->SBxphiP, 1, MPIU_SCALAR, MPIU_SUM, PETSC_COMM_WORLD);
    		...
    		/*----计算最后的方程------------------*/ 
	VecAXPY(user->Y,ts->dt/3*user->SBxphiP,user->X);
...
PetscScalar boundary(PetscScalar mx,PetscScalar LapI)
{
	if(LapI>mx-1){return (LapI-mx);}
	else{
		if(LapI<0){return (LapI+mx);}
		else{return LapI;}
	}
}
